#let tx_out_height_estimate(input) = {
  ...
  let storage = if "storage" in input { input.storage.len() } else { 0 }
  return (address + value + storage) * 8pt
}

#let storage_field(indent, k, val) = [
  #if val == "" [
    #h(indent)\+ #raw(k)
  ] else [
    #h(indent)\+ #raw(k):
      ...
      #stack(dir: ttb, spacing: 0.4em,
        for item in val [
          #storage_field(indent + 1.2em, "", item) \
        ]
      )
    ]
      ...
      #stack(dir: ttb, spacing: 0em,
        for (k, v) in val.pairs() [
          #storage_field(indent + 1.2em, k, v) \
        ]
      )
    ]
  ]
...
#let tx_out(input, position, inputHeight, styles) = {
  ...
  let storage = if "storage" in input [
    *storage:* \ 
    #v(-0.8em)
    #stack(dir: ttb, spacing: 0.4em,
      ..input.storage.pairs().map(((k,val)) => storage_field(1.2em, k, val))
    )
  ] else []
  ...
  let storageHeight = measure(storage, styles).height + if "storage" in input { 6pt } else { 0pt }
  let thisHeight = 32pt + addressHeight + valueHeight + storageHeight
  return (
    content: place(dx: position.x, dy: position.y, [
      ...
      *#input.name*
      ...
      #if "storage" in input { place(dx: 0em, dy: addressHeight + valueHeight)[#storage] }
    ]),
    height: thisHeight,
  )
}
